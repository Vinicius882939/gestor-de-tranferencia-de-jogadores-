...
def guardar_ficheiro():
    with open(FICHEIRO, "w") as f:
        json.dump(transferencias, f, indent=4)
    return (200, "Transferencias guardadas com sucesso.")


# CARREGAR
def carregar_ficheiro():
    global transferencias, proximo_id
    try:
        with open(FICHEIRO, "r") as f:
            transferencias = json.load(f)
        if len(transferencias) > 0:
            proximo_id = max(t["id"] for t in transferencias) + 1
        return (200, "Transferencias carregadas com sucesso.")
